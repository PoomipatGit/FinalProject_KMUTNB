...
class BatteryTestpage(BasePage):
	...
	def update_loop(self):
		...
		if self.running_time:
			...
			run_time_str = str(td)[:-5] if len(str(td)) > 5 else str(td)
			self.run_time_sec.set(f"{run_time_str}")
	...
	def change_mode(self, mode):
		if mode == "Charge":
			self.selected_mode.set("Mode: Charge")
			print("Mode: Charge")
		elif mode == "Discharge":
			self.selected_mode.set("Mode: Discharge")
			print("Mode: Discharge")
	...
	def cutoffcap(self):
		self.cutoff_cap_entry = tk.Entry(
			self.setup_container, 
	...
	def cutofftime(self):
		self.cutoff_time_entry = tk.Entry(
			self.setup_container, 
	...
	def cutoffvolt(self):
		self.cutoff_volt_entry = tk.Entry(
			self.setup_container, 
	...
	def cutoffcurr(self):
		self.cutoff_curr_entry = tk.Entry(
			self.setup_container, 
	...
	def cutoffengy(self):
		self.cutoff_engy_entry = tk.Entry(
			self.setup_container, 
